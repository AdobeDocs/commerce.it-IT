...
Queste note sulla versione descrivono le versioni più recenti di [!DNL Payment Services].

Adobe fornisce supporto per l’ultima versione rilasciata di Payments Services. Vengono fornite a titolo di riferimento le note sulla versione per le versioni precedenti.
...
![Nuovo problema](../assets/new.svg)<!-- Issue PAY-6374 --> Ora il pulsante **Impostazioni** nel dashboard [!DNL Payment Services] viene reindirizzato alla pagina di configurazione dell&#39;amministratore per i metodi di pagamento, fornendo un flusso di lavoro più semplice per la gestione delle impostazioni di pagamento.

_19 gennaio 2026_

![Nuovo problema](../assets/new.svg)<!-- Issue PAY-6325 --> Ora gli esercenti possono visualizzare l&#39;ID venditore PayPal come colonna nel report [transazioni](reporting.md#transactions-report-view) per identificare le transazioni inserite da un particolare cliente.
...
![Nuovo problema](../assets/new.svg)<!-- Issue PAY-5658 --> Ora gli esercenti possono filtrare le transazioni in base ai dettagli di pagamento nel report [transazioni](reporting.md#transactions-report-view) per ottenere dati più dettagliati e accurati sui metodi di pagamento.

_15 luglio 2024_

![Nuovo problema](../assets/new.svg)<!-- Issue PAY-5571 --> Ora gli esercenti possono filtrare le transazioni in base all&#39;e-mail del cliente Commerce nel [report delle transazioni](reporting.md#transactions-report-view). Inserisci l’e-mail del cliente per filtrare le transazioni per quella specifica e-mail.

_9 luglio 2024_

![Nuovo problema](../assets/new.svg)<!-- Issue PAY-5488 --> Ora gli esercenti possono visualizzare l&#39;ID cliente Commerce come colonna nel [report delle transazioni](reporting.md#transactions-report-view) per identificare le transazioni inserite da un particolare cliente. Inoltre, gli esercenti possono filtrare il rapporto transazioni in base a questo ID cliente Commerce per gli ordini associati.
...
![Nuovo problema](../assets/fix.svg)<!-- Issue PAY-4888 --> Ora gli esercenti possono filtrare le transazioni con carta di credito e di debito in base alle ultime quattro cifre del numero di carta nel [report Transazioni](reporting.md#transactions-report-view).
...
![Nuovo](../assets/new.svg)<!-- Issue PAY-4288 --> Ora gli esercenti possono [configurare _solo_ pulsanti di pagamento PayPal](payments-options.md#use-only-paypal-payment-buttons) e _non_ utilizzare l&#39;opzione di pagamento PayPal con carta di credito. Questo consente agli esercenti di fornire varie opzioni di pagamento, tra cui i pulsanti di pagamento Venmo e PayPal, e di utilizzare un provider di carte di credito esistente invece dell&#39;opzione di pagamento PayPal con carta di credito.

![Nuovo](../assets/new.svg)<!-- Issue PAY-4050 --> aggiunta [visualizzazione visualizzazione dati](payouts.md#payouts-data-visualization-view), visualizzata nella Home page del servizio di pagamento, per il report sullo stato del pagamento dell&#39;ordine.
...
![Nuovo](../assets/new.svg)<!-- Issue PAY-3705 --> `increment_id` è ora disponibile per l&#39;utilizzo nella riconciliazione dei pagamenti nei sistemi ERP esterni. Viene propagato a `custom_id` _e_ `invoice_id`, visibile sia nel webhook PayPal che nei dettagli dell&#39;attività esercente per un pagamento.
...
![Nuovo](../assets/new.svg)<!-- Issue PAY-3420 --> Apple Pay è ora disponibile come Smart Button PayPal. Questa [opzione di pagamento](payments-options.md#apple-pay-button) consente ai clienti di utilizzare la funzionalità Touch ID sul dispositivo iOS o macOS per selezionare Apple Pay. Apple Pay elabora il pagamento utilizzando le credenziali di pagamento della carta di credito e di debito memorizzate sul dispositivo.

_28 giugno 2021_

![Nuove](../assets/new.svg)<!-- Issue PAY-1720 --> controversie per gli ordini del punto vendita sono ora disponibili in [Report stato pagamento ordine](order-payment-status.md#view-disputes). Puoi risolvere le controversie navigando direttamente al Centro di risoluzione PayPal da [!DNL Payment Services].
...
![Nuovo](../assets/new.svg)<!-- Issue PAY-2761 --> È ora possibile personalizzare i dati visualizzati nel [report sullo stato dei pagamenti dell&#39;ordine](order-payment-status.md#show-and-hide-columns) e nel [report sui pagamenti](payouts.md#show-and-hide-columns) visualizzando o nascondendo le colonne tramite il controllo Impostazioni colonna.
...
![È stato risolto un problema](../assets/fix.svg)<!-- PAY-6769 --> che causava un problema intermittente nelle vetrine dei sistemi EDS che utilizzavano i pulsanti avanzati PayPal e che impediva la disponibilità di alcuni metodi di pagamento nella pagina di pagamento.

## v2.16.0

_27 luglio 2026_

![Nuovo problema](../assets/new.svg)<!-- Issue PAY-6510 --> I commercianti possono ora eseguire in autonomia l&#39;onboarding di un altro conto PayPal nell&#39;ambito del sito Web direttamente da [!DNL Commerce Admin]. [!DNL Payment Services] Home include ora un pulsante **[!UICONTROL Connect a different PayPal account for a website]** che reindirizza alla pagina di configurazione dell&#39;amministratore per i metodi di pagamento. Per ulteriori informazioni, consulta [Collegare un altro conto PayPal per un sito Web](connect-website-account.md).
...
![Nuovo](../assets/new.svg)<!-- PAY-6400 --> Aggiunta di una configurazione **[!UICONTROL Line Items Enabled]** per abilitare o disabilitare l&#39;invio di elementi di riga e di suddivisione dell&#39;importo a PayPal. Questa impostazione consente di risolvere i problemi di pagamento quando estensioni di terze parti aggiungono tariffe personalizzate non supportate da [!DNL Payment Services]. Per ulteriori informazioni, vedere [Elementi riga](line-items.md).

![Nuovo](../assets/new.svg)<!-- PAY-6457 --> aggiunto [Pulsanti di pagamento rapido all&#39;inizio dell&#39;estrazione](payments-options.md#express-checkout-buttons) per favorire l&#39;accelerazione delle operazioni di pagamento.

![Nuovo](../assets/new.svg)<!-- PAY-6458 --> aggiunto supporto per [Metodi di pagamento locali](payments-options.md#local-payment-methods) per le opzioni di pagamento locali e specifiche per l&#39;area geografica. I commercianti possono abilitare o disabilitare i moduli LPM disponibili direttamente in [Commerce](configure-admin.md#local-payment-methods).

![Nuovo](../assets/new.svg)<!-- PAY-6377 --> Aggiunta una configurazione sandbox [Paese dell&#39;acquirente](sandbox.md#buyers-country) per simulare l&#39;ubicazione dell&#39;acquirente e controllare i metodi di pagamento di cui è stato eseguito il rendering.

![È stato risolto il problema](../assets/fix.svg)<!-- PAY-6458 --> Migliorata la compatibilità con i filtri antifrode di PayPal passando i dati dell&#39;indirizzo e-mail dell&#39;acquirente, migliorando le funzionalità di protezione antifrode.

![È stato risolto un problema](../assets/fix.svg)<!-- PAY-6413 --> che impediva il completamento dell&#39;estrazione PayPal per i prodotti virtuali a causa di un indirizzo di spedizione mancante.

![È stato risolto un problema](../assets/fix.svg)<!-- PAY-6450 --> che impediva la corretta gestione degli errori di checkout del pulsante PayPal nelle pagine dei dettagli del prodotto, nei minicart, nel carrello e nella parte superiore del pagamento.

![È stato risolto il problema](../assets/fix.svg)<!-- PAY-6453 -->. Il flusso dei callback di spedizione lato server durante l&#39;estrazione di GraphQL è stato disattivato per evitare conflitti durante l&#39;estrazione.

![Problema risolto](../assets/fix.svg)<!-- PAY-6456 --> È stata aggiornata la descrizione della configurazione di [Apple Pay](configure-admin.md#apple-pay) per chiarire che Apple rifiuta i pagamenti da domini non verificati. I commercianti devono verificare tutti i domini in cui vengono utilizzati i pulsanti Apple Pay. Si applicano restrizioni nazionali.

## v2.13.3

_14 gennaio 2026_

![È stato risolto un problema](../assets/fix.svg)<!-- PAY-6399 --> che causava l&#39;utilizzo di una scheda con Vault errata durante un&#39;estrazione amministratore. Ora, la scheda selezionata viene utilizzata correttamente quando si effettua un ordine.

## v2.13.2

_5 gennaio 2026_

![È stato risolto il problema](../assets/fix.svg)<!-- PAY-6390 --> Miglioramenti e ottimizzazioni generali mediante l&#39;implementazione della minimizzazione JS migliorata.

## v2.13.1

_18 dicembre 2025_

![È stato risolto un problema](../assets/fix.svg)<!-- PAY-6355 --> che impediva agli esercenti di selezionare un metodo di spedizione durante l&#39;estrazione.

![È stato risolto il problema](../assets/fix.svg)<!-- PAY-6347 --> Migliorata la resilienza dell&#39;estrazione rimuovendo le chiamate API non necessarie.

![Problema risolto](../assets/fix.svg)<!-- PAY-6368 --> È stato implementato un meccanismo di fallback per il caricamento di Payments SDK quando il dominio principale non è disponibile, garantendo l&#39;accesso ai campi della carta di credito durante l&#39;estrazione.
...
![Nuovo](../assets/new.svg)<!-- PAY-xxxx --> Ora, [!DNL Payment Services] supporta la modale **One-Time Checkout (OTC)** di PayPal, con informazioni di contatto e consegna incorporate.

![Nuova](../assets/new.svg)<!-- PAY-xxxx --> esperienza mobile migliorata tramite **autenticazione con cambio app PayPal**, insieme a una migliore integrazione API per percorsi di utenti più fluidi. Questa funzionalità è disponibile solo per i clienti statunitensi basati su [!DNL Payment Services].

![New](../assets/new.svg)<!-- PAY-xxxx --> ha aggiunto il supporto per l&#39;autenticazione **3D Secure (3DS)** affinché Fastlane soddisfi i **requisiti per l&#39;autenticazione avanzata del cliente (SCA)**. Questo consente ai commercianti di [!DNL Payment Services] nel Regno Unito e nell&#39;UE di elaborare le transazioni con **autenticazione 3DS**, migliorando la prevenzione delle frodi e garantendo la conformità alle normative regionali.

![Nuovo](../assets/new.svg)<!-- PAY-xxxx --> Ora, [!DNL Payment Services] commercianti possono scegliere tra **temi chiari e scuri** per il componente di pagamento Fastlane, consentendo alle pagine di pagamento di corrispondere alla progettazione del sito. Se gli stili personalizzati non soddisfano gli standard di accessibilità, il sistema ripristina automaticamente le impostazioni predefinite.

![Problema risolto](../assets/fix.svg)<!-- PAY-xxxx --> Problema del caricatore risolto durante l&#39;estrazione da **admin con 3DS challenge**.

![È stato risolto il problema](../assets/fix.svg)<!-- PAY-xxxx --> **è stata migliorata la convalida durante l&#39;archiviazione della configurazione del pagamento** tramite API.
...
![Problema risolto](../assets/fix.svg)<!-- PAY-6164 --> Ora, [!DNL Payment Services] utilizza la valuta di base per i metodi di spedizione disponibili nel [servizio di chiamata di spedizione lato server PayPal (SSSC)](https://developer.paypal.com/docs/multiparty/checkout/standard/customize/shipping-module/).
...
![Nuovo](../assets/new.svg)<!-- PAY-6227 --> Durante il check-out con PayPal, [!DNL Payment Services] ignora la finestra a comparsa di conferma dell&#39;ordine per un processo di acquisto più rapido. Questa funzione è abilitata dal [callback di spedizione lato server](payments-options.md#server-side-shipping-callbacks-for-paypal-payment-buttons), che calcola la spedizione e i totali direttamente nella pagina di revisione di PayPal.
...
![Nuovo](../assets/new.svg)<!-- PAY-5938 --> Ora [!DNL Payment Services] consente ai commercianti di gestire le impostazioni di pagamento per massimizzare la flessibilità della loro attività. Questa versione migliora la possibilità di allegare [più account PayPal](configure-admin.md#use-multiple-paypal-accounts) per le aree geografiche e i marchi supportati da un commerciante. Il nostro team di vendita può fornire un collegamento di onboarding per configurare gli ambiti di visualizzazione del sito web e del negozio.
...
![Nuovo](../assets/fix.svg)<!-- PAY-5789 --> [!DNL Payment Services] ora supporta [l&#39;autenticazione protetta 3D con Google Pay](security.md#3ds), migliorando la sicurezza per gli esercenti e i clienti durante le transazioni di pagamento.

![Correzione](../assets/fix.svg)<!-- PAY-5703 --> [!DNL Payment Services] aggiunge la possibilità per [clienti di salvare le schede direttamente nel proprio **Account personale**](vaulting.md#vaulting-without-purchase), migliorando la comodità e semplificando le future estrazioni. `Vault without purchase functionality might not be 100% compatible with Adobe Commerce 2.4.4 due to a known issue with` [`GraphQL authorization mechanisms`](https://developer.adobe.com/commerce/webapi/graphql/usage/authorization-tokens/).
...
![Correzione](../assets/fix.svg)<!-- PAY-5792 --> [!DNL Payment Services] visualizza ora le descrizioni e gli indirizzi di fatturazione per [carte in deposito nella pagina di pagamento](vaulting.md), dando ai clienti maggiore visibilità sui metodi di pagamento salvati.
...
![Correzione](../assets/fix.svg)<!-- PAY-5492 --> è stato risolto un problema che si verificava quando un commerciante aggiungeva termini e condizioni personalizzati alla pagina di pagamento. Durante un [pagamento rapido](payments-options.md#standard-vs-advanced-payments-experience) un acquirente ora può accettare questi termini e condizioni per completare il pagamento senza alcun problema.
...
![I nuovi](../assets/new.svg)<!-- PAY-4868 --> commercianti possono [configurare Google Pay durante l&#39;intera esperienza di acquisto](payments-options.md), in modo analogo ad altri pulsanti di pagamento in [!DNL Payment Services] tramite l&#39;amministratore.

![Nuovo](../assets/new.svg)<!-- PAY-4381 --> [I servizi di pagamento supportano Google Pay tramite GraphQL](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/), consentendo agli esercenti di avere un&#39;esperienza Commerce headless con il metodo di pagamento Google Pay.

![Nuovo](../assets/new.svg)<!-- PAY-4878 --> Ora la funzionalità di estrazione di base di [!DNL Payment Services] è inclusa nel bundle per gli esercenti Adobe Commerce e Magento Open Source.[!DNL Payment Services] è ora possibile supportare i commercianti con aziende in una qualsiasi delle 200 aree geografiche del mondo.[!DNL Payment Services] Il pagamento di base fornisce le opzioni di addebito/credito, PayPal, Venmo (se disponibile) e PayLater (se disponibile) in un onboarding self-service.

![Correzione](../assets/fix.svg)<!-- PAY-5291 --> La ricezione della conferma del pagamento per alcune transazioni potrebbe essere ritardata. In questo caso, ora i commercianti possono ottenere uno stato di pagamento aggiornato per un ordine. [I servizi di pagamento rilevano lo stato in sospeso di una transazione di pagamento](order-payment-status.md#payment-status-updates) in un ordine rilevando le transazioni in sospeso e monitorandole in modo proattivo e aggiornando quando lo stato in sospeso viene acquisito.
...
- dalla [visualizzazione dettagli ordine](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/order-management/orders/order-processing#view-an-order) in Commerce Admin.
...
![Nuovo](../assets/new.svg)<!-- PAY-4381 --> [Payment Services ora supporta l&#39;integrazione con GraphQL](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/). Con il supporto GraphQL per i pulsanti di pagamento PayPal, i campi ospitati e Apple Pay,[!DNL Payment Services] ora supporta una configurazione di Commerce headless.
...
![Nuovo](../assets/new.svg)<!-- PAY-4638 --> Aggiunta integrazione [con Signifyd](fraud-protection.md), che fornisce servizi automatizzati di protezione dalle frodi.

![Nuovo](../assets/new.svg)<!-- PAY-3981 --> [Apple Pay è stato promosso a un&#39;opzione di pagamento separata](payments-options.md#apple-pay-button), al di fuori dei pulsanti di pagamento PayPal, per aumentare la visibilità dell&#39;opzione di pagamento da parte dell&#39;acquirente e consentire agli esercenti di controllare il posizionamento e lo stile di Apple Pay.
...
![Nuovo](../assets/new.svg)<!-- Issue xxx --> Aggiunta [disponibilità nei seguenti paesi e nelle valute associate](compatibility.md#standard-vs-advanced-payment-services-experience): Australia, Francia, Regno Unito.

![Nuovo](../assets/new.svg)<!-- Issue PAY-4296 --> aggiunte [risorse espanse per i ruoli di amministratore](configure-admin.md#configure-roles) per garantire che gli utenti amministratori possano creare e gestire gli ordini per i clienti e possano visualizzare[!DNL Payment Services] nel menu Vendite.

![Nuovo](../assets/new.svg)<!-- Issue PAY-4236 --> aggiunto [annullamento automatico per gli ordini che generano errori durante l&#39;estrazione](checkout.md#order-auto-voided-if-error).

![Nuovo](../assets/new.svg)<!-- Issue PAY-4183 --> funzionalità creata per [visualizzare il pulsante di opzione per il pagamento con carta di credito](payments-options.md#paypal-debit-or-credit-card-button) nella pagina di pagamento.
...
![Correzione](../assets/fix.svg)<!-- Issue PAY-4226 --> è stato risolto un problema che impediva ai nuovi commercianti [!DNL Payment Services] di utilizzare l&#39;estrazione in Admin.[!DNL Payment Services] utilizzava in precedenza l’ID cliente Commerce, che non esiste per i nuovi clienti.

![Correzione](../assets/fix.svg)<!-- Issue PAY-4205 --> è stato risolto un problema che causava la sostituzione dello stato dell&#39;indirizzo di spedizione specificato con lo stato nelle impostazioni fiscali predefinite durante l&#39;estrazione tramite l&#39;opzione [PayPal](payments-options.md#paypal-payment-buttons). Ora i clienti possono far spedire i loro ordini in uno stato diverso da quello configurato come predefinito nelle impostazioni fiscali dell’esercente.

![Correzione](../assets/fix.svg)<!-- Issue PAY-4202 --> è stato risolto un problema che impediva ai clienti di utilizzare il vaulting delle carte per completare un acquisto o eliminare un metodo di pagamento con vaulting per un negozio [utilizzando l&#39;azione di pagamento `Authorize and Capture`](production.md#set-payment-services-as-payment-method). In precedenza, quando il cliente tentava di utilizzare o modificare le proprie carte di credito archiviate, veniva visualizzato l’errore &quot;Provider Vault ID not found&quot; (ID archivio provider non trovato).
...
![Nuovo](../assets/new.svg)<!-- Issue PAY-784 -->[!DNL Payment Services] ora include la possibilità di impostare un account esercente per [utilizzare più account business PayPal](configure-admin.md#use-multiple-paypal-accounts). Questo consente al commerciante di gestire i negozi in più paesi utilizzando valute diverse o di utilizzare Adobe Commerce per una parte della tua attività.

![Nuovi](../assets/new.svg)<!-- Issue PAY-3231 --> I commercianti possono [aggiungere un [!UICONTROL Soft Descriptor]](configure-admin.md) a siti Web o a singole configurazioni di visualizzazioni dello store che vengono visualizzate nei rendiconti bancari delle transazioni dei clienti per delineare marchi, store o linee di prodotti.

![Nuovo](../assets/new.svg)<!-- Issue PAY-3707 --> [Attiva o disattiva i campi della carta di credito e i pulsanti di pagamento PayPal](configure-admin.md#paypal-payment-buttons) per l&#39;estrazione nelle impostazioni [!DNL Payment Services].
...
![Problema noto](../assets/bug.svg)<!-- Issue PAY-0 --> [!DNL Payment Services] non è compatibile con `commerce-data-export` v101.20 e versioni successive, pertanto è incompatibile con [[!DNL Channel manager] l&#39;estensione](https://experienceleague.adobe.com/en/docs/discontinued/using/commerce-channels) (interrotta).
...
![Nuovo](../assets/new.svg)<!-- Issue PAY-2682 --> L&#39;estensione [!DNL Payment Services] per [!DNL Adobe Commerce] e [!DNL Magento Open Source] è ora disponibile per i commercianti canadesi. Gli esercenti possono visualizzare la configurazione dei pagamenti in [francese](compatibility.md##standard-vs-advanced-payment-services-experience) o [inglese](compatibility.md#standard-vs-advanced-payment-services-experience).

![Nuovo](../assets/new.svg)<!-- Issue PAY-2681 --> [!DNL Payment Services] supporta [Dollari canadesi (CAD)](compatibility.md#standard-vs-advanced-payment-services-experience) per le carte di credito e le transazioni PayPal.
...
![Problema risolto](../assets/fix.svg)<!-- Issue PAY-2277 --> Ora puoi [abilitare o disabilitare in modo selettivo i singoli pulsanti di pagamento PayPal](configure-admin.md#paypal-payment-buttons).
...
![Problema noto](../assets/bug.svg)<!-- Issue PAY-2842 --> Le transazioni con carta di credito [di prova potrebbero non riuscire con PayPal](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26825) durante l&#39;elaborazione dei pagamenti in un ambiente sandbox.
...
![Problema noto](../assets/bug.svg)<!-- Issue PAY-2473 --> L&#39;utilizzo di [chiavi di composizione non corrette](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26826) durante l&#39;installazione dell&#39;estensione impedisce all&#39;utente di [autenticarsi](https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/prerequisites/authentication-keys) con `MAGEID` corretto.

![Problema noto](../assets/bug.svg)<!-- Issue PAY-2474 --> [!DNL Payment Services] report [&#x200B; potrebbero non essere sincronizzati immediatamente](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26837).

![Problema noto](../assets/bug.svg)<!-- Issue PAY-2475 --> Il tuo [account sandbox PayPal](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26836) per [!DNL Payment Services] non può essere verificato se crei tale account durante l&#39;onboarding.
