...
Quando un ordine contiene articoli nominati per diverse origini di magazzino, la pagina [!DNL Commerce Admin] [!UICONTROL **Selezione Source**] ora ti guida automaticamente attraverso la selezione dell&#39;origine appropriata per ciascun articolo. <!-- ACCS-1832 -->

### Eseguire una query sui dettagli della disponibilità dell&#39;origine per la vetrina

La query GraphQL [`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability) ora restituisce informazioni aggiuntive per gli storefront:

* Ogni origine include il relativo `name` e un flag `is_pickup_location_active`, pertanto gli storefront possono visualizzare i nomi delle origini e identificare i percorsi di prelievo attivi nel negozio. <!-- ACCS-1614 -->

* Il tipo `SourceAvailability` ora include `is_saleable` e `backorder_message` campi, pertanto le vetrine possono gestire [!UICONTROL Add to Cart] in base alla vendita effettiva anziché alle scorte fisiche. <!-- ACCS-1858 -->

### Recuperare le autorizzazioni del cliente

Un nuovo endpoint REST [!DNL Commerce] (`GET /V1/customers/:customerId/companyRoles`) restituisce tutti i ruoli e le autorizzazioni aziendali assegnati a un cliente. Il tipo GraphQL `CompanyBasicInfo` ora include anche `role_id`, `role_name` e i dati delle autorizzazioni, pertanto le integrazioni possono recuperare le assegnazioni del ruolo aziendale di un utente in tutti gli account associati in una singola richiesta. <!-- ACCS-1617 -->

### Iscriviti a un webhook di salvataggio fattura

Il webhook `observer.sales_order_invoice_save_after` consente di eseguire la logica dopo il salvataggio di una fattura. <!-- CEXT-6706 -->

### Visualizza informazioni sull&#39;origine nominata

Quando un ordine contiene articoli con un&#39;origine di magazzino designata, [!DNL Commerce Admin] contrassegna tali articoli nella pagina di visualizzazione dell&#39;ordine e nella schermata di selezione dell&#39;origine della spedizione, in modo che i commercianti possano evadere gli ordini dall&#39;origine corretta. <!-- ACCS-941 -->
...
* Carrello ottimizzato e caricamento prezzo livello di pagamento per evitare problemi con un numero elevato di cataloghi condivisi. <!-- ACCS-1150 -->

* È stato risolto un problema a causa del quale le modifiche all’attributo del file non venivano rispettate. <!-- CCSAAS-5395 -->

* È stato risolto un problema a causa del quale i dati di sostituzione del prodotto potevano essere incoerenti. <!-- ACCS-1844 -->

* È stato risolto un problema che poteva causare un errore 401 in modo intermittente nelle richieste API REST simultanee. <!-- CCSAAS-5417 -->

* È stato risolto un problema a causa del quale la data di inizio o di fine di una regola del prezzo del carrello poteva funzionare in modo imprevisto quando non veniva impostata un’ora esplicita. <!-- ACCS-1856 -->

* È stato risolto un problema a causa del quale una regola del prezzo del carrello accettava date di inizio o fine inesistenti. <!-- ACCS-1902 -->

* È stato risolto un problema che impediva il salvataggio della configurazione di [!UICONTROL Catalog]. <!-- CCSAAS-5436 -->

* È stato risolto un problema che poteva verificarsi durante l&#39;esecuzione di richieste simultanee all&#39;API di importazione (`POST /V1/import/json`). <!-- ACCS-1053 -->

* È stato risolto un problema a causa del quale la quantità vendibile poteva generare rapporti eccessivi sulla disponibilità per i prodotti con un&#39;origine magazzino designata. <!-- ACCS-1103 -->

* È stato risolto un problema a causa del quale l&#39;API dei commenti dell&#39;ordine (`POST /V1/orders/{id}/comments`) non rispettava la distinzione tra maiuscole e minuscole. <!-- ACCS-1752 -->

{{accs-release}}

>[!ENDSHADEBOX]

## Settembre 2026 - #1 sulla versione
...
I seguenti elementi sono stati pubblicati in Produzione l’8 settembre 2026.

>[!BEGINSHADEBOX]

### Adobe Commerce as a Cloud Service aggiornato alla versione 2.4.9
...
I nuovi endpoint API REST [`GET` e `PUT /V1/system/config`](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/system-config) consentono alle integrazioni di leggere e aggiornare i valori di configurazione del sistema Commerce, tra cui:

* Informazioni archivio
* Impostazioni di spedizione e imposte
* Impostazioni metodo di pagamento
* Impostazioni aziendali e B2B

Questi endpoint consentono agli amministratori di sincronizzare la configurazione tra gli ambienti a livello di programmazione anziché riconfigurare manualmente [!DNL Commerce Admin]. L&#39;esecuzione di `GET /V1/system/config` nell&#39;ambiente sandbox e quindi dell&#39;esecuzione di `PUT /V1/system/config` con la configurazione sandbox recuperata in precedenza consente di sincronizzare le modifiche alla configurazione da Sandbox a Produzione. <!-- ACCS-607, CCSAAS-5346 -->

### Eseguire query sulla disponibilità dell’inventario tramite GraphQL
...
La query GraphQL [`storeConfig`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-persistent-cart-and-account-sharing-configuration) ora restituisce i valori di configurazione `persistent_enabled`, `persistent_shopping_cart`, `persistent_options_wishlist` e `share_customer_accounts_scope`, pertanto gli storefront possono accedere alle impostazioni del carrello acquisti permanente e della lista dei desideri senza contattare il supporto tecnico. <!-- USF-4051 -->

### Cerca gli ordini dei clienti per prodotto, SKU o ID ordine

L&#39;input di GraphQL [`CustomerOrdersFilterInput`](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/customer) ora supporta un campo `search` facoltativo che corrisponde al numero d&#39;ordine, allo SKU dell&#39;elemento o al nome dell&#39;elemento, in combinazione con qualsiasi altro filtro fornito. <!-- USF-4290 -->

### Aggiornare ed eliminare modelli e-mail personalizzati tramite l’API

I nuovi endpoint `PUT` e `DELETE` [e-mail personalizzate](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/custom-email/) consentono alle integrazioni di aggiornare ed eliminare modelli e-mail personalizzati. <!-- CCSAAS-5091 -->

### Visualizzare gruppi di opzioni e identificatori del prodotto tramite l’API REST

La chiamata REST `GET /V1/products/:sku/options` ora restituisce `group` e `option_uids` campi per ogni opzione, corrispondenti agli identificatori già disponibili tramite GraphQL. <!-- ACCS-1370 -->

### Nuovi eventi di catalogo condiviso

I seguenti eventi di catalogo condiviso sono ora disponibili per l&#39;abbonamento a utilizzando [!DNL Adobe I/O Events]:<!-- ACCS-1532 -->

* Assegnazione categoria (`observer.shared_catalog_assign_categories`)
* Annullamento assegnazione categoria (`observer.shared_catalog_unassign_categories`)
* Assegnazione società (`plugin.magento.shared_catalog.api.company_management.assign_companies`)
* Annullamento assegnazione società (`plugin.magento.shared_catalog.api.company_management.unassign_companies`)
* Revoca assegnazione società a tutti (`plugin.magento.shared_catalog.api.company_management.unassign_all_companies`)
* Salvataggio del catalogo condiviso (`plugin.magento.shared_catalog.api.shared_catalog_repository.save`)
* Eliminazione catalogo condiviso (`plugin.magento.shared_catalog.api.shared_catalog_repository.delete`)

### Utilizza gli indirizzi della società per ordini fornitore, preventivi e restituzioni

La Rubrica aziendale è ora integrata con flussi di lavoro B2B aggiuntivi. Le aziende che utilizzano una rubrica condivisa visualizzano indirizzi coerenti e con ambito aziendale in:
...
Per informazioni dettagliate, incluse le mutazioni di GraphQL e gli endpoint REST, vedere il [log delle modifiche del pacchetto B2B di compatibilità storefront](https://experienceleague.adobe.com/en/tools/commerce-storefront/releases/changelog/#storefront-compatibility-b2b-package-v1-0-24-2026-07-20).

<!-- USF-3629, USF-4187, USF-4188, USF-4189, USF-4191, USF-4192, USF-4193, USF-4194, USF-4195 -->

### Nascondi immagini dalle visualizzazioni dello store in [!DNL AEM Assets]

L&#39;integrazione di [!DNL AEM Assets] ora supporta un parametro `hiddenStoreViews`, in modo che le immagini importate possano essere incluse come nascoste in specifiche visualizzazioni archivio. Questo consente di mostrare diverse immagini dei prodotti a diversi punti vendita regionali o demografici. <!-- ACAP-1308 -->
...
### Registra le modifiche degli ordini nella cronologia degli ordini

>[!IMPORTANT]
>
>Questa funzione è disabilitata per impostazione predefinita. Per abilitarlo, contatta il tuo Customer Success Manager Adobe Commerce o crea un ticket di supporto.

Quando un ordine viene modificato, Commerce può ora aggiungere un commento leggibile alla cronologia del nuovo ordine che riepiloga le modifiche apportate rispetto all’ordine sostituito. <!-- ACCS-1157 -->

### Miglioramenti e correzioni di bug

In questa versione sono inclusi i miglioramenti, le ottimizzazioni e le correzioni di bug seguenti:

* È stato risolto un problema che poteva verificarsi quando gli ordini degli ospiti venivano effettuati utilizzando l’e-mail di un cliente registrato. <!-- CCSAAS-5313 -->

* È stato risolto un problema che causava problemi di risorse a causa delle ripetute esecuzioni dell’esportazione dei dati. <!-- CCSAAS-5275 -->

* È stato risolto un problema di fallback con le etichette della raccolta multimediale di GraphQL nell&#39;integrazione [!DNL AEM Assets]. <!-- ACAP-1308 -->

* È stato risolto un problema relativo ai parametri SDK [!DNL PayPal] che poteva influire sul rendering di estrazione. <!-- PAY-6961 -->

* È stato risolto un problema che causava la visualizzazione di metodi di pagamento [!DNL Payment Services] non supportati al momento dell&#39;estrazione. <!-- PAY-6976 -->

* È stato risolto un problema a causa del quale i payload degli eventi con campi con valori array, come gli eventi di assegnazione di società e categorie di catalogo condivisi, potevano contenere oggetti vuoti invece dei dati previsti. <!-- CEXT-6554 -->

* È stato risolto un problema che poteva causare errori di visualizzazione durante la configurazione di diversi attributi dell’indirizzo del cliente o del cliente ricercabili. La griglia clienti ora notifica quando raggiungi il limite. <!-- CCSAAS-5303 -->

* È stato risolto un problema che causava il mancato funzionamento degli URL dell’immagine della categoria nel feed di esportazione dei dati della categoria. <!-- ACCS-1571 -->

* È stato risolto un problema che poteva causare errori intermittenti durante l’assegnazione o la revoca dell’assegnazione concomitante di prodotti tra cataloghi condivisi diversi. <!-- CCSAAS-5287 -->

* Se si imposta l&#39;opzione **Usa in Opzioni di ricerca** su No, l&#39;attributo rimane visibile come colonna della griglia, ma non viene conteggiato per il limite di 16 colonne. <!-- CCSAAS-5370 -->

* È stato risolto un problema che poteva interessare la navigazione dell’amministratore. <!-- CCSAAS-5232 -->

* È stato risolto un problema a causa del quale i carrelli vuoti potevano riportare totali diversi da zero. <!-- ACCS-1730 -->

>[!ENDSHADEBOX]

## Agosto 2026 - #1 sulla versione
...
### Iscriviti a un evento per i pagamenti delle fatture

Quando si registra un pagamento di una fattura viene emesso un nuovo evento `observer.sales_order_invoice_pay`, in modo che le integrazioni possano sottoscrivere l&#39;evento anziché eseguire il polling per le modifiche dello stato della fattura. <!-- CEXT-5983 -->

### Miglioramenti e correzioni di bug

In questa versione sono inclusi i miglioramenti, le ottimizzazioni e le correzioni di bug seguenti:

* È stato risolto un problema che causava un rallentamento nel recupero delle aziende assegnate da un cliente tramite GraphQL. <!-- ACCS-1425 -->
...
I seguenti elementi sono disponibili negli ambienti di produzione dal 28 luglio 2026.

>[!BEGINSHADEBOX]

### Modifica ordini con REST

>[!IMPORTANT]
>
>Questa funzione è disabilitata per impostazione predefinita. Per abilitarlo, contatta il tuo Customer Success Manager Adobe Commerce o crea un ticket di supporto.

I nuovi [endpoint REST API](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/order-management/) replicano la funzionalità [!DNL Commerce Admin] [!UICONTROL **Modifica ordine**], consentendo alle integrazioni di modificare un ordine a livello di programmazione:

| Metodo | Endpoint | Descrizione |
| --- | --- | --- |
| `POST` | `/V1/orders/{orderId}/edit/start` | Copia l’ordine in un nuovo carrello modificabile e restituisce l’ID del carrello. |
| `POST` | `/V1/orders/{orderId}/edit/submit` | Sottomettere il carrello modificato come nuovo ordine e annullare l&#39;ordine originale. |

Dopo aver chiamato `edit/start`, modifica il carrello restituito utilizzando gli endpoint REST del carrello standard, quindi chiama `edit/submit`. Il nuovo ordine eredita il metodo di pagamento dell&#39;ordine originale a meno che non venga sostituito dal carrello e venga creato come sostituzione collegata dell&#39;originale annullato. Entrambi gli endpoint richiedono la risorsa ACL `Magento_Sales::actions_edit`. <!-- ACCS-1284 -->
...
### Importa più codici coupon per file

Il limite per l’importazione di coupon in blocco per file può essere modificato contattando il Customer Success Manager di Adobe Commerce o creando un ticket di supporto. <!-- CCSAAS-5176 -->

### Gestire i modelli e-mail personalizzati tramite l’API

I seguenti nuovi endpoint REST API consentono alle integrazioni di elencare, recuperare e creare [modelli e-mail personalizzati](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/custom-email/):

| Metodo | Endpoint | Descrizione |
| --- | --- | --- |
| `GET` | `/V1/custom-email/templates` | Elenca i modelli e-mail personalizzati, restituendo l’ID, il codice, l’oggetto e il tipo di ciascun modello. |
| `GET` | `/V1/custom-email/templates/{id}` | Recuperate un singolo modello, inclusi il corpo e gli stili. |
| `POST` | `/V1/custom-email/templates` | Crea un modello e-mail personalizzato e restituisce il relativo ID assegnato dal server. |
...
I nuovi endpoint REST API [`orderChain`](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/order-management/) consentono alle integrazioni di modificare un ordine utilizzando il relativo ID e risolvere automaticamente l&#39;intera catena di ordini modificati:
...
### Cerca nella griglia dell&#39;ordine in base ai valori degli attributi personalizzati

>[!IMPORTANT]
>
>Questa funzione è disabilitata per impostazione predefinita. Per abilitarlo, contatta il tuo Customer Success Manager Adobe Commerce o crea un ticket di supporto.

I commercianti possono ora filtrare la griglia ordini [!DNL Commerce Admin] in base ai valori memorizzati negli attributi personalizzati dell&#39;ordine. Un filtro [!UICONTROL **Attributi personalizzati**] è disponibile nella riga del filtro della griglia dell&#39;ordine.<!-- ACCS-923 -->

### Imposta un&#39;origine magazzino designata per gli articoli del carrello

La nuova mutazione GraphQL `setNominatedSourceOnCartItems` assegna un&#39;origine inventario specifica agli articoli del carrello, supportando scenari quali il prelievo in negozio (BOPIS) e la spedizione dallo store. La mutazione accetta un `cart_id` e un elenco di elementi, ciascuno con un `cart_item_uid` e un `source_code`, e restituisce qualsiasi `rejected_items` con un codice di errore strutturato: `UNKNOWN_SOURCE`, `SOURCE_DISABLED`, `NOT_ENOUGH_QTY` o `SKU_SOURCE_CONFLICT`. Ogni SKU in un carrello viene risolta in un&#39;unica origine nominata e il passaggio di un valore `source_code` nullo o vuoto cancella la nomina. <!-- ACCS-932 -->

### Iscriviti a un evento per i carrelli che corrispondono alle regole dei promemoria

Un nuovo evento `observer.reminder_matched_carts` viene emesso dopo che le regole di promemoria e-mail hanno eseguito la logica corrispondente, riportando le informazioni sui carrelli corrispondenti. Le integrazioni possono abbonarsi a questo evento e inoltrare i dati a un sistema esterno, ad esempio una piattaforma di marketing, invece di affidarsi alle e-mail di promemoria native. <!-- CCSAAS-5173 -->
...
Nella pagina dei dettagli dell&#39;ordine [!DNL Commerce Admin] viene ora visualizzata la sequenza di modifica completa per un ordine che include l&#39;ordine originale e tutti gli ordini figlio creati tramite modifiche successive. I commercianti possono spostarsi tra gli ordini, attivare o disattivare la visibilità degli ordini annullati e accedere a tutte le fatture, le spedizioni, le note di accredito e i commenti degli ordini associati dalla vista sequenza.<!-- ACCS-968 -->
...
* Migliorate le prestazioni dell&#39;integrazione di [!DNL AEM Assets]. <!-- ACAP-1242 -->

* È stato corretto un errore che poteva verificarsi durante l&#39;aggiunta di un semplice SKU di prodotto a un prodotto configurabile in [!DNL Commerce Admin]. <!-- ACCS-1132 -->

* È stato risolto un problema a causa del quale la coda di messaggi poteva interrompere l’elaborazione dei nuovi messaggi in caso di accumulo di troppi record obsoleti. <!-- ACCS-1292 -->

* È stato risolto un problema che impediva la creazione dell’ordine di amministrazione con l’errore &quot;SKU non disponibile nel catalogo condiviso&quot;. <!-- ACCS-1318 -->

* È stato risolto un arresto anomalo che si verificava durante la creazione o la modifica di prodotti in bundle. <!-- CCSAAS-5211 -->

* È stato risolto un problema a causa del quale il posizionamento dell&#39;ordine non impegnava le scorte nell&#39;origine designata per gli articoli che utilizzavano il prelievo in-store o la spedizione dal punto vendita. <!-- ACCS-1374 -->

* Le tariffe personalizzate non aggiornate vengono ora cancellate dalla risposta alla query del carrello. <!-- ACCS-1400 -->

* È stato risolto un problema nell&#39;integrazione [!DNL AEM Assets] a causa del quale gli attributi product asset-role perdevano dati locali durante l&#39;esportazione del catalogo. <!-- ACCS-1401 -->

* È stato migliorato l&#39;avviso ricevuto durante il salvataggio di un&#39;integrazione che indica che [!DNL Dynamic Media] non è abilitato. <!-- ACAP-1298 -->

* I campi Nome evento e Alias ora vengono convalidati in minuscolo quando si sottoscrive un evento. <!-- CEXT-6164 -->

* I pattern di regole regex del webhook vengono ora convalidati quando si salva un webhook condizionale. <!-- CEXT-6287 -->
...
## #1 sulla versione di maggio 2026
...
* È stato risolto un problema che ha causato la mancata visualizzazione di `custom_attributes` nello schema REST per entità quali Ordine, Carrello, Fattura, Nota di credito e Società. <!-- CCSAAS-4818 -->
...
È ora possibile utilizzare gli strumenti per sviluppatori di codifica [AI](https://developer.adobe.com/commerce/extensibility/developer-agent/coding-tools/){target="_blank"} per creare nuove applicazioni [!DNL App Builder] e convertire le estensioni PHP [!DNL Adobe Commerce] esistenti in applicazioni [!DNL App Builder]. Sono disponibili i seguenti tutorial per dimostrare come utilizzare gli strumenti:
...
* [!DNL Commerce Storefront on Edge Delivery Services] ora include [componenti di eliminazione B2B](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/). Sono ora disponibili i seguenti menu a discesa B2B:

  * **[Gestione società](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/company-management/)** - Abilita la gestione del profilo società e le autorizzazioni basate sui ruoli per le vetrine di Adobe Commerce.
  * **[Commutatore società](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/company-switcher/)** - Fornisce un componente dell&#39;interfaccia utente che consente agli utenti di passare da un&#39;azienda all&#39;altra a cui sono associati.
  * **[Ordini di acquisto](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/purchase-order/)** - Gestisce i flussi di lavoro degli ordini di acquisto, le regole di approvazione e la cronologia degli ordini di acquisto per le transazioni B2B.
  * **[Gestione dei preventivi](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/quote-management/)** - Abilita i preventivi negoziabili per i clienti B2B con flussi di lavoro di richiesta, negoziazione e approvazione.
  * **[Elenchi di richieste](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/requisition-list/)** - Fornisce gli strumenti per la creazione e la gestione degli elenchi di richieste per acquisti ripetuti e ordini in blocco.
...
* È stato risolto un problema a causa del quale gli attributi personalizzati del prodotto non venivano aggiornati tramite l&#39;API REST quando [!DNL AEM Assets integration] era abilitato e il prodotto conteneva immagini. <!-- ACAP-1178 -->
...
* [Gestione utenti](./user-management.md) — È stato modificato il ruolo di **Amministratore prodotto** in Admin Console per aggiornare automaticamente l&#39;accesso degli utenti all&#39;Amministratore Commerce. <!-- CCSAAS-3012 -->
