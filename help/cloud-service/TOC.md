---
user-guide-title: '[!DNL Adobe Commerce as a Cloud Service]'
user-guide-description: Questa guida fornisce istruzioni dettagliate per l'utilizzo di  [!DNL Adobe Commerce as a Cloud Service].
feature: Services, Saas
recommendations: noDisplay, noCatalog
source-git-commit: f6667afbc2ba6bc323b1fecee8921d3ee572879f
workflow-type: tm+mt
source-wordcount: '50'
ht-degree: 10%

---


# [!DNL Adobe Commerce as a Cloud Service] {#cloud-service}

- [Adobe Commerce as a Cloud Service](overview.md)
- [Confronto delle funzioni](feature-comparison.md)
- [Introduzione](getting-started.md)
- [Gestione di utenti e identità](user-management.md)
- [Vetrina](storefront.md)
- [Gestione risorse](assets-management.md)
- [Casi d’uso](use-cases.md)
- [Responsabilità condivisa](shared-responsibility.md)
- [Esercitazione sull’estensione delle valutazioni](workbook.md)
- [Prerequisiti per l’estensione delle valutazioni](workbook-prerequisites.md)
- Migrazione {#migration}
   - [Panoramica](migration/overview.md)
   - [Strumento di migrazione dati in blocco](migration/bulk-data.md)
   - [Strumenti di codifica](migration/coding-tools.md)
- [Note sulla versione](release-notes.md)
- [Torna alla home page dei servizi di Commerce](https://experienceleague.adobe.com/en/docs/commerce/user-guides/home)
