...
È disponibile il supporto per l’ultima versione di Live Search rilasciata. Vengono fornite a titolo di riferimento le note sulla versione per le versioni precedenti.
...
![Nuove](../assets/new.svg) regole di merchandising per la ricerca, regole predefinite e [regole di merchandising per le categorie](./category-merch.md) ora includono **[!UICONTROL Intelligent Ranking Boost]** per verificare in che modo le strategie di classificazione intelligenti influenzano i risultati in base alla rilevanza testuale. L’anteprima delle regole corrisponde all’incremento configurato. [Ulteriori informazioni](./rules-add.md#intelligent-ranking-boost).
...
[!BADGE Supportato]{type=Informative tooltip="Supportato"} Adobe Commerce versioni 2.4.5 e successive

![Correzione](../assets/fix.svg) è stato risolto un problema che impediva il funzionamento della funzionalità Aggiungi al carrello nella pagina di elenco prodotti con il widget PLP in alcuni casi isolati.
...
![Correzione](../assets/fix.svg) è stato corretto un errore che si verificava in determinate condizioni relative alla funzionalità dell&#39;estensione Visual Merchandiser.
...
![Nuova versione GA](../assets/new.svg) per le seguenti funzionalità di ricerca avanzate: ricerca a più livelli con `startsWith` e `contains`. [Ulteriori informazioni](workspace.md#layered-search-and-expansion-of-search-types).
![Correzione](../assets/fix.svg) L&#39;oggetto `ProductInterface` nel servizio [Live Search](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search) è stato dichiarato obsoleto. Utilizzare invece l&#39;oggetto `ProductView` nel servizio catalogo.
...
![Correzione](../assets/fix.svg) È stata corretta la query GraphQL [`productSearch`](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search#filtering-by-categories) per filtrare correttamente in base a `categoryPath` e `categoryList` per le categorie.
...
![Correzione](../assets/fix.svg) è stato risolto un problema a causa del quale il popover di ricerca visualizzava il simbolo di valuta sempre prima del valore del prezzo, indipendentemente dalle impostazioni internazionali.
...
![Correzione](../assets/fix.svg) La dimensione della tabella dei prodotti e dei feed di prezzo è stata notevolmente ridotta. Le tabelle `catalog_data_exporter_products` e `catalog_data_exporter_product_prices` hanno subito una riduzione sostanziale delle dimensioni.
...
- 14 giorni
- 30 giorni
![Correzione](../assets/fix.svg) &#39;Eventi&#39; (Boost/Pin/Bury/Hide) è stato rinominato in &#39;Classificazione manuale&#39;.
![Correzione](../assets/fix.svg) &#39;Tipo di classificazione&#39; è stato rinominato in &#39;Classificazione intelligente&#39;.
![Correzione](../assets/fix.svg) di bug minori.
...
![Correzione](../assets/fix.svg) [Collegamento Ricerca avanzata](https://experienceleague.adobe.com/it/docs/commerce-admin/catalog/catalog/search/search) rimosso dal piè di pagina della vetrina.
...
![Correzione](../assets/fix.svg) [L&#39;attributo di prezzo personalizzato](https://experienceleague.adobe.com/it/docs/commerce-admin/catalog/product-attributes/attributes-input-types) non restituisce più un errore se configurato come [facet](facets-add.md).
![Correzione](../assets/fix.svg) è stato risolto un problema che causava un errore se non era disponibile alcun [simbolo di valuta](https://experienceleague.adobe.com/it/docs/commerce-admin/stores-sales/site-store/currency/currency-configuration#step-5-customize-currency-symbols-optional) (`data-currency-symbol`).
![Correzione](../assets/fix.svg) [[!DNL Storefront popover]](storefront-popover.md) ora mostra il [Prezzo speciale](https://experienceleague.adobe.com/it/docs/commerce-admin/catalog/products/pricing/product-price-special) (prezzo finale minimo) quando disponibile.
...
![Correzione](../assets/fix.svg) Nella vetrina, il collegamento a piè di pagina [Termini di ricerca](https://experienceleague.adobe.com/it/docs/commerce-admin/catalog/catalog/search/search-terms) è stato rimosso e la relativa cache è disabilitata per le installazioni di [!DNL Live Search].
...
![Correggi](../assets/fix.svg) colonne ordinabili nell&#39;editor sinonimi.
...
![Bug](../assets/bug.svg) Il servizio [!DNL Live Search] supporta solo la [valuta di base](https://experienceleague.adobe.com/it/docs/commerce-admin/stores-sales/site-store/currency/currency-configuration) dell&#39;installazione di Adobe Commerce.
