...
Adobe Commerce [!DNL Live Search] e [[!DNL Catalog Service]](../catalog-service/guide-overview.md) collaborano per fornire una soluzione di ricerca efficiente, pertinente e intuitiva. Questa soluzione consente ai clienti di trovare rapidamente ciò di cui hanno bisogno. In particolare, [!DNL Catalog Service] fa emergere i dati del catalogo per i servizi SaaS, ad esempio [!DNL Live Search] da utilizzare.
...
- A partire da [!DNL Live Search] 4.0.0, la scheda di ricerca è stata [obsoleta](release-notes.md#live-search-400).
...
>
>Per informazioni dettagliate, vedere [Preparazione HIPAA in Adobe Commerce](https://experienceleague.adobe.com/en/docs/commerce-admin/start/compliance/hipaa-ready-service/overview) e la guida [Operazioni](https://experienceleague.adobe.com/en/docs/commerce-admin/start/compliance/hipaa-ready-service/operations), che elenca Live Search tra i servizi Commerce non conformi HIPAA.

## Panoramica del flusso di lavoro

Per l&#39;onboarding di [!DNL Live Search] è necessario:

1. [Installa](#install) l&#39;estensione [!DNL Live Search]
1. [Configurare](#configure) le chiavi API
1. [Sincronizza](#sync) i dati del catalogo
1. [Verifica](#verify) che i dati del catalogo siano stati esportati
1. [Configura](#configuredata) i dati
1. [Verifica](#test) la connessione
1. [Verifica](#capture) che gli eventi acquisiscano i dati
1. [Personalizza](#customize) la vetrina

## &#x200B;1. Installa l&#39;estensione [!DNL Live Search] {#install}
...
1. Disattivare [!DNL OpenSearch] e i moduli correlati e installare [!DNL Live Search]. [!DNL OpenSearch] e [!DNL Live Search] non possono essere entrambi abilitati nella stessa istanza Commerce.
...
Dopo aver verificato gli indicizzatori, il passaggio successivo consiste nel [configurare le chiavi API](#configure).
...
>[!NOTE]
>
>L&#39;impostazione *Admin* > _[!UICONTROL Stores]_> [!UICONTROL Settings] >_[!UICONTROL Configuration]_ > **[!UICONTROL Live Search]** > **[!UICONTROL Storefront Features]** > **[!UICONTROL Enable Product Listing Widgets]** controlla solo i widget dell&#39;elenco di prodotti. Nessuna impostazione di *Admin* per disabilitare l&#39;esperienza completa della vetrina [!DNL Live Search] (ad esempio il popover di ricerca). Utilizzare i comandi del modulo CLI in questa procedura per mantenere attiva la ricerca della vetrina esistente durante la configurazione di [!DNL Live Search].
...
1. Disattivare i moduli storefront [!DNL Live Search] mantenendo attivato `Magento_LiveSearchAdapter`.

   ```bash
   bin/magento module:disable Magento_LiveSearchStorefrontPopover Magento_LiveSearchProductListing
   ```

   [!DNL Elasticsearch] continua a gestire le richieste di ricerca dalla vetrina mentre il servizio [!DNL Live Search] sincronizza i dati del catalogo e indicizza i prodotti in background. Se si mantiene abilitato `Magento_LiveSearchAdapter`, la ricerca nella vetrina non viene spostata su [!DNL Live Search]. È sufficiente che il modulo rimanga abilitato in modo che le dipendenze del motore di ricerca di Commerce continuino a essere risolte correttamente.

   >[!IMPORTANT]
   >
   >Mantieni `Magento_LiveSearchAdapter` abilitato durante questa fase, anche se era [obsoleto](release-notes.md#live-search-400) a partire da [!DNL Live Search] 4.0.0. `Magento\Search\Model\EngineResolver` dipende dall&#39;attivazione di questo modulo, pertanto la disattivazione interrompe la ricerca della vetrina esistente con un errore `500`. Impossibile disabilitare `Magento_LiveSearchAdapter` anche se `Magento_LiveSearchMetrics` è abilitato, perché `composer.json` del modulo `Magento_LiveSearchMetrics` dichiara una dipendenza da `Magento_LiveSearchAdapter`. Non è necessario disabilitare `Magento_LiveSearchMetrics` per questo flusso di lavoro.
...
1. Abilitare l&#39;estensione [!DNL Live Search] e disabilitare [!DNL OpenSearch] (moduli Magento Elasticsearch e OpenSearch).
   ...
   >[!NOTE]
   >
   >[!DNL OpenSearch] e [!DNL Live Search] non possono essere entrambi abilitati nella stessa istanza Commerce. Il comando disable include l&#39;elenco dei moduli di Commerce che supportano OpenSearch. Se nell&#39;istanza di Commerce non è installato un modulo, verrà visualizzato un errore `module does not exist`.
...
## &#x200B;2. Configurare le chiavi API {#configure}
...
## &#x200B;3. Sincronizza dati catalogo {#sync}

[!DNL Live Search] sposta i dati del catalogo nell&#39;infrastruttura SaaS di Adobe. I dati vengono indicizzati e i risultati della ricerca vengono consegnati da questo indice direttamente alla vetrina. A seconda delle dimensioni e della complessità, l’indicizzazione può richiedere da 30 minuti a diverse ore.
...
## &#x200B;4. Verifica che i dati siano stati esportati {#verify}

È possibile verificare i dati del catalogo esportati da Adobe Commerce nel database e verificare che i dati siano stati sincronizzati correttamente in [!DNL Live Search] utilizzando l&#39;area di lavoro di GraphQL [!DNL Live Search].

- Dal database, utilizzare le query SQL per cercare le voci nelle tabelle seguenti:
...
## &#x200B;5. Configurare i dati {#configuredata}
...
## &#x200B;6. Verifica la connessione {#test}
...
## &#x200B;8. Personalizza per la vetrina {#customize}
...
Poiché [!DNL Live Search] non ha accesso al database completo dei prodotti, le API principali di GraphQL e Commerce per [!DNL Live Search] non dispongono della parità completa.
...
- [Campo [!DNL Live Search]](https://github.com/adobe/storefront-search-as-you-type)

La raccolta automatica dei dati di interazione dell’utente non funziona per impostazione predefinita se non si utilizzano i componenti standard come Search Adapter, widget Luma o widget CIF di AEM. Adobe AI utilizza i dati raccolti per il merchandising intelligente e il tracciamento delle prestazioni. Per risolvere questo problema, devi sviluppare una soluzione personalizzata per implementare questa raccolta dati in modo headless.
...
| Giapponese | Giappone | ja_JP | ja_JP |
...
Se il widget rileva che l’impostazione della lingua di amministrazione di Commerce corrisponde a una lingua supportata, per impostazione predefinita viene utilizzata tale lingua. In caso contrario, il widget utilizza per impostazione predefinita l’inglese. In Amministrazione, l&#39;impostazione della lingua viene configurata passando a _[!UICONTROL Stores]_> [!UICONTROL Settings] >_[!UICONTROL Configuration]_ > _[!UICONTROL General]_> [!UICONTROL Country Options].
...
Se necessario, installare il modulo `module-data-services-graphql` che espande la copertura GraphQL esistente dell&#39;applicazione per includere i campi necessari per la raccolta dei dati comportamentali della vetrina.
...
Per migliorare la funzionalità di ricerca, [!DNL Live Search] raccoglie i dati di interazione dell&#39;utente e memorizza tali informazioni nei cookie del browser. Questa raccolta di dati richiede il consenso degli utenti quando sono abilitate le restrizioni sui cookie. [!DNL Live Search] e [!DNL Product Recommendations] condividono lo stesso meccanismo di raccolta dati e la stessa gestione dei cookie. Per ulteriori informazioni sulle restrizioni dei cookie e sulla conformità alla privacy, consulta [Gestire le restrizioni dei cookie](../product-recommendations/setting-cookie.md).
